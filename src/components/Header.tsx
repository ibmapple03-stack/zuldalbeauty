...
export default function Header() {
  ...
  return (
    <header className="sticky top-0 z-40 border-b border-brand-black/10 bg-brand-cream/95 backdrop-blur">
      <div className="bg-brand-brown text-brand-white">
      ...
      <div className="container-page flex items-center justify-between gap-4 py-3">
        ...
        <div className="flex items-center gap-2">
          <Link
            href="/cart"
            className="relative inline-flex items-center gap-1.5 rounded-full bg-brand-brown px-4 py-2 text-xs font-accent font-medium text-brand-white hover:bg-brand-gold transition-colors"
          >
            <Icon name="cart" className="h-4 w-4" />
            <span className="hidden sm:inline">Cart</span>
            {totalItems > 0 && (
              <span className="ml-1 flex h-5 min-w-5 items-center justify-center rounded-full bg-brand-gold px-1 text-[10px] font-semibold text-brand-black">
                {totalItems}
              </span>
            )}
          </Link>
